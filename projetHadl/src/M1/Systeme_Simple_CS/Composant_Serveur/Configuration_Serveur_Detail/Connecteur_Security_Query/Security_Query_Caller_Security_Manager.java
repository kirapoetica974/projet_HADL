package M1.Systeme_Simple_CS.Composant_Serveur.Configuration_Serveur_Detail.Connecteur_Security_Query;

import M2.Objet_Architectural.Interface_Communication.Role_Requis;

public class Security_Query_Caller_Security_Manager extends Role_Requis {

	public Security_Query_Caller_Security_Manager() {
		super();
		this.setNom("Security_Query_Caller_Security_Manager");
	}
}
